...
<div class="gallery">
    <div class="container">
        <h3><span>房源列表</span></h3>
        <p class="autem">以下为满足搜索条件的出全部房源。</p>
        ...
        <ul class="pagination pagination-lg">
            <li class="disabled"><a <c:if test="${pageNum-1 >0}" >href="${pageContext.request.contextPath }/house/Web_list?pageNum=${pageNum-1}"</c:if>><i class="fa fa-angle-left">&laquo;</i></a></li>
            <c:forEach var="prePageNum" begin="1" end="${total/pageSize+1}">
            <li <c:if test="${prePageNum == pageNum}" >class="active" </c:if> ><a href="${pageContext.request.contextPath }/house/Web_list?pageNum=${prePageNum}">${prePageNum}</a></li>
            </c:forEach>
            <li><a <c:if test="${pageNum+1 < total/pageSize+1}" >href="${pageContext.request.contextPath }/house/Web_list?pageNum=${pageNum+1}"</c:if>><i class="fa fa-angle-right">&raquo;</i></a></li>
        </ul>
        <!-- //page -->
        <div class="clearfix"> </div>
    </div>
